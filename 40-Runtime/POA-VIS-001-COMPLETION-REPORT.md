...
- **`bash.exe.stackdump` investigated and resolved.** A stray `bash.exe.stackdump` was present at the repository root, already untracked before any command in this session ran. Investigation (file birth timestamp, stack trace, loaded-module list, repo-wide search for related artifacts) established:
  - **Timestamp**: created 2026-08-10 12:50:07, before this session's first tool call (`git status` at conversation start already showed it as untracked) — it predates the POA-VIS-001 build and is not a byproduct of it.
  - **Cause**: the stack trace is entirely inside `msys-2.0.dll` (MSYS2's `fork()` emulation), with `windhawk.dll` present in the loaded-module list — a third-party, system-wide DLL-injection framework (Windhawk) installed on this machine, unrelated to Git Bash, Node.js, Next.js, or Claude Code. This is the well-documented MSYS2-on-Windows failure mode where a globally-injected DLL rebases the process address space and breaks `fork()`. No Node.js/npm/Next.js module, and no Claude Code binary, appears anywhere in the crash's loaded-module list.
  - **Scope**: a repo-wide search (`**/*.stackdump`, `**/*.{dmp,core,log}`) found no other crash or dump artifacts; the only log found (`30-Products/poa-vis-001/.next/dev/logs/next-development.log`) is normal Next.js dev-server output inside the already-gitignored `.next/` build directory.
  - **Conclusion**: unrelated environmental tooling debris — a Windows/MSYS2/Windhawk interaction on the local machine, not a POA application or Claude Code environment defect. No application or build fix was required.
  - **Resolution**: the stray file was deleted and `*.stackdump` was added to a new repository-root `.gitignore` so any future occurrence is excluded automatically rather than requiring manual cleanup.
