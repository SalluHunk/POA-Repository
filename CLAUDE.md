...
`GAP-001` (`POA-PRS-001` vs. the repository's identifier convention) was formally **CLOSED** by Commander decision, 2026-08-23 (GAP-001 Identifier Convention Decision Record, `POA-ADR-001.md`; Resolution A — existing `POA-<FAMILY>-<NNN>` convention retained, `POA-PRS-001` not adopted, no migration performed) — see `40-Runtime/PRS-001-CONFLICT-DOSSIER.md` for the original conflict record and `20-Shared/GOV/GAP-REGISTER-001.md` §6 for the closure record. No repository identifier or directory may be renamed, moved, or restructured without a dedicated future governance mission resolving it, regardless of what other work is in progress.
